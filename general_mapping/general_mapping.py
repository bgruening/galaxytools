...
def single_grep(args):
    ...
    subprocess.call(["grep", "-w", "-i", "%s" % pattern, mapping], stdout=open(tfile,'w'))
...
if __name__ == "__main__" :
    parser = argparse.ArgumentParser(description='Small wrapper around grep to filter same mapping files.')

    parser.add_argument("-m", "--mapping", dest="mapping_path",
                      required=True,
                      help="Path to the Mapping file.")

    parser.add_argument("-q", "--query", dest="query_path",
                      required=True,
    ...
    parser.add_argument("-c", "--column-with-id", dest="column",
                      default="c1",
