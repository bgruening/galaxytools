...
def main():
    ...
    parser.add_argument(
        "--iformat", required=True, help="Specify the input file format."
    )
    ...
    parser.add_argument(
        ...
        dest="detailed",
        action="store_true",
        help="Returns more detailed output",
    )
            ...
            if args.detailed:
                out = np.column_stack(
                    (
                        ...
                        np.round(
                            np.array(metabs[entry]["SyGMa_score"], dtype=float),
                            decimals=5,
                        ),  # score rounded to 5 dp
                        Chem.rdMolDescriptors.CalcMolFormula(
                            Chem.MolFromSmiles(smiles)
                        ),  # Molecular formula
                        len(
                            metabs[entry]["SyGMa_pathway"].split("\n")
    ...
    if args.detailed:
        np.savetxt(
            ...
            fmt="%s",
            delimiter="\t",
            header="smiles\tcompound_id\tsygma_score\tmolecular_formula\tsygma_n\tsygma_pathway",
            comments="",
        )
    else:
        np.savetxt(
            ...
            fmt="%s",
            delimiter="\t",
            header="smiles\tcompound_id\tsygma_score",
            comments="",
        )
