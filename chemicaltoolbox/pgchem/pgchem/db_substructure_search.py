...
def parse_command_line(argv):
    ...
    parser.add_argument('--istyle', required=True, choices=['pattern', 'infile'], help='input is file containing multiple molecules as SMILES string or a unique SMILES string pattern?')
    parser.add_argument('-i', '--input', required=True, help='input file name or SMILES patters')
    ...
    parser.add_argument('-max', type=int, default=10000, help='Maximum number of retrieved molecules')
    parser.add_argument('-dbname', required=True, help='Specify the name of the db to connect to')
    parser.add_argument('-dbuser', required=True, help='Specify the user name for the connection to the db')
    parser.add_argument('-dbhost', required=True, help='Specify the host for the db connection')
    parser.add_argument('-dbpasswd', required=True, help='Specify the password for the connection to the db')
    parser.add_argument('-lib', required=True, help='Name of the libname where the compounds are stored')
...
def create_query( args, mol ):
    ...
    return ("""SELECT *
                ...
                LIMIT %(max_results)i
            """
            % { 'libname' : args.lib, 'mol': mol.write("can").split()[0].strip().replace('\\', "\\\\"), 'max_results': args.max } )
