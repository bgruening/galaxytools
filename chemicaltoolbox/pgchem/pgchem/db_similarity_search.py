...
def parse_command_line(argv):
    ...
    parser.add_argument('--istyle', required=True, type=str, choices=['pattern', 'infile'], help='input is file containing multiple molecules as SMILES string or a unique SMILES string pattern?')
    parser.add_argument('-i', '--input', required=True, type=str, help='input file name or SMILES patters')
    parser.add_argument('-o', '--output', type=str, default='output.sdf', help='output file name')
    parser.add_argument('-oformat', required=True, type=str, help='output file format')
    parser.add_argument('-tanimoto', type=float, default=0.85, help='Minimum Tanimoto threshold')
    parser.add_argument('-dbname', type=str, required=True, help='Specify the name of the db to connect to')
    parser.add_argument('-dbuser', type=str, required=True, help='Specify the user name for the connection to the db')
    parser.add_argument('-dbhost', type=str, required=True, help='Specify the host for the db connection')
    parser.add_argument('-dbpasswd', type=str, required=True, help='Specify the password for the connection to the db')
    parser.add_argument('-lib', type=str, required=True, help='Name of the libname where the compounds are stored')
    parser.add_argument('-fetch', type=str, help='Specify query to perform')
    parser.add_argument('--header', type=bool, help='Do you want to include the header as the first line of the output table?')
...
def create_query( args, mol ):
    ...
    return ("""SELECT *, mol @ E'%(mol)s'::molecule as tani
                ...
                ORDER BY tani desc
            """
            % { 'libname' : args.lib, 'mol': mol.write('can').split()[0].strip().replace('\\', "\\\\"), 'Tanimoto': args.tanimoto } )
