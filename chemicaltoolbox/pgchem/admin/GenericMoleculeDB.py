...
class Molecule(Base):
    ...
    id          = Column(Integer, primary_key=True) # unique_id will be auto-generated, and linked to its chembl code in the synonyms table
...
class Synonym(Base):
    ...
    __table_args__  = (
    ForeignKeyConstraint(['fk_molecule_id'], ['tbl_molecule.id'], onupdate="CASCADE", ondelete="CASCADE"),
...
def create_indices(con, schema):
    try:
        ...
        connection.execute("""CREATE INDEX idx_mol
                    ON %s.tbl_molecule
                    USING gist(mol);""" % schema)
        connection.close()
        engine.dispose()
    except:
