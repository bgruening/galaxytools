...
class GenericMoleculeParser():
    ...
    def parse_into_database(self):
        for mol in pybel.readfile(self.molecule_type, self.filepath):
            ...
            syn_obj_list = list()
            [ syn_obj_list.append(GenericMoleculeDB.Synonym(key, value)) for key, value in synonyms.items() ]

            DBMolecule.synonyms = syn_obj_list # map(GenericMoleculeParser.Synonym, synonyms.items())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog = 'Molecule Parser and pgchem insertion script.')
    parser.add_argument('--version', action='version', version='%(prog)s 0.1')
    parser.add_argument('--processes', '-p', dest="processes", type=int,
        default=4, help='# of processes to use for file parsing')
    parser.add_argument('--filetype', '-f', dest="filetype", default='smi')
    parser.add_argument('--molecule-file', '-m', dest="compound_file", 
        required=True,
