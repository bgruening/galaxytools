...
def manage_loc_file(path, data):
    ...
    """
    new_config_entry = '%s\t%s\t%s\tmoleculedb\t%s\tlocalhost\t%s' % (data['db_key'], data['description'], data['schema'], data['user'], data['password'])
    lines = list()
    replace = False
...
loc_file = os.path.join(sys.argv[5], 'tool-data', 'cheminformatics_chemical_searches_db.loc')

if sys.argv[1] == '--non-admin-user':
    error_text = "The user %s have no permissions to insert data in the database, please contact the administrator or check your settings in universe_wsgi.ini." % sys.argv[2]
...
config_data = {'user': user, 'password': password, 'db_key': sys.argv[2], 'schema': sys.argv[2], 'description': sys.argv[6]}
manage_loc_file(loc_file, config_data)

try:
    abs_exec_path = os.path.abspath(executeable)
    args = [abs_exec_path, '--processes', processes, '--molecule-file', sys.argv[1], '--filetype', sys.argv[7]]
...
if return_code:
    ...
    sys.stdout.write(stdout)
    outfile.write( 'The file %s was sucessfully inserted into the database.\nPlease remember to check your .loc file under ./tool-data/ and restart galaxy.\n' % (sys.argv[2]) )
    if stderr:
        outfile.write('The following structures failed to import to the database:\n%s' % (stderr))
    outfile.close()
