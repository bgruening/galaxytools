...
def get_files(options):
    ...
    with open(options.out, "wb+") as out:
        if options.whitelist:
            allowed_extensions = [
                ext.strip() for ext in unescape(options.whitelist).split("\n")
            ]
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""Download compressed files and extract files of with chosen extensions
    """
    )
