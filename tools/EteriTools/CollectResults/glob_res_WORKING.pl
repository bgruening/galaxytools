...
my $results_merge_overlap = 0.51;
my $merge_overlap = 0.51;
my $results_min_cluster_size = 3;
#my $results_partition_type  =hard
...
my $evaluate_min_overlap  = 0.51;
my $evaluate_class0_as_fp = 1;
  ...
  #  print "read $key... $file\n" if ($verbose);
    ...
    if ( !exists $exist_part_used{$key} ) {
      ...
      $exist_part_used{$key} = 1;
        print "exist_part_used = $exist_part_used{$key} \n";
