...
my $part_file_soft = 0;
my $part_file_cm = 0;
my ($tabularFiles, $round, $round_last, $merge_cluster_ol, $merge_overlap, $min_cluster_size, $cm_min_bitscore, $cm_max_eval, $cm_bitscore_sig, $part_file_soft, $part_file_cm) = @ARGV;
...
print "options loaded for glob_results.pl \n";

print "part file soft = $part_file_soft \n";
print "part file cm = $part_file_cm \n";
...
if ( $part_file_soft != 0 &&  $part_file_cm != 0) {

  print "henc et ifi mej \n";

  $exist_part = read_partition($part_file_soft);

  my $used_cm = read_partition($part_file_cm);
...
#   $exist_part = read_partition("$rootDir/RESULTS/partitions/final_partition.soft");
#
#   my $used_cm = read_partition("$rootDir/RESULTS/partitions/final_partition.used_cmsearch");
#   map { $exist_part_used{ $_->[0] } = 1 } @{$used_cm};
# }
...
foreach my $file (@tabFiles ) {
  ...
  if ( !exists $exist_part_used{$key} ) {
    ...
    print "elsi mej, read_CM_tabfile_ext chi anum \n";
  }
  ...
  } ##end of if ( -e "$rootDir/FASTA/data.greylist" )
...
foreach my $round ( 1 .. $round_max ) {
  ...
  print "\nmake unique hits out of " . scalar( @{$all_hits} ) . "..." ;
...
print (" finished glob_results.pl");
...
sub evaluate_cm_hits {
  ...
  my $classOLs = fragment_overlap( $hits, $classHash, $min_overlap );
  ...
  delete $hit_class_counts{"0"} if ( $evaluate_class0_as_fp == 0 );
...
sub get_f_measure {
  ...
  foreach my $idx (@clus) {
    ...
    my $clus_class = evaluate_cm_hits( $class_Frags, \@hits, $min_class_OL );
