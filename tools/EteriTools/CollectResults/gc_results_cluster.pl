...
#print "options loaded for alignCenter.pl";



my $results_top_num = 10; ##later change for param
...
print "results_partition_type $part_file\n";
...
my @fa_scan = read_fasta_file("$in_root_dir/FASTA/data.fasta.scan");
...
#    my @model_fa = read_fasta_file("$in_root_dir/CLUSTER/$orig_cl.cluster/MODEL/model.tree.fa");
...
#    }
  ...
  # if ($evaluate) {
  #   close(EVAL);
  #   makeCol("$clus_dir/cluster.all.eval");
  # }
...
sub getHitMap {
  ...
  ###################################
  my $all_frags = read_fragments("$rootDir/FASTA/data.map");
