...
my $OPTS_locarna_maligs  = " -p $p --max-diff-am $max_diff_am --tau $tau --max-diff $max_diff --alifold-consensus-dp ";
my $OPTS_locarna_p_model = " -p $p --max-diff-am $max_diff_am --tau $tau --max-diff $max_diff --struct-weight 180 --plfold-span 150 --plfold-winsize 200 --temperature 180 --mea-beta 400 --consistency-transformation ";
...
computeTreeAligsLocP( "maligs.loc_new", $center_fa_file, $tree_file, $center_subtree_min, $center_subtree_max, $use_sim_stree, $use_prob_alig, $mloc_opts, $dp_dir, "SUBTREES" );

my $in_eval_mode = 0;

my $cluster_id = "MISS";
print "cluster id befor if = $cluster_id \n";

# if ( $in_cluster_dir =~ /\/(\d+)\.(\d+)\.cluster/ ) {
#   $cluster_id = "$1.$2";
#   print "cluster id from if = $cluster_id \n";
# }
print "cluster id out of if = $cluster_id \n";
my $tree_dir = "Tree";
...
my @subtrees_final = ();
print "rank = $rank \n";
print "subtrees_final = @subtrees_final \n";
...
print "Found " . scalar(@subtrees_final) . " models. Build model alignment now for best subtree... \n";
my $model_dir = "MODEL";
system("mkdir -p $model_dir");


my @fa_center = read_fasta_file($center_fa_file);
my $subset_fa = writeSubsetFasta( \@fa_center, $subtrees_final[0]->{NAMES}, "$model_dir/model.tree.fa", 1 );
...
sub computeTreeAligsLocP {
  ...
  foreach my $id ( sort { $a <=> $b } keys %{$subtrees_simtree_href} ) {
     ...
     my $subset_fa = writeSubsetFasta( \@fa_tree, $childs, "$tgtDir/$id/$id.fa", 1 );
  ...
  #
    ...
    if ( !$use_probabilistic ) {
      ...
      foreach my $key ( @{$childs} ) {
        system("ln -f  $dpDir/$key $loc_pp_dir/$key") if ( -e "$dpDir/$key" );
      }
    }
    ...
    print "call command = $call \n";

    if ($use_probabilistic) {
      print "probabilistici  ifi mej \n";
      $call .= "--probabilistic " if ( $call !~ /--probabilistic/ );
      system( $call . " $tgtDir/$id/$id.fa 1>$tgtDir/log/$id.locarna_call 2>$tgtDir/log/$id.locarna_call_err", $in_verbose )
        if ( !-e "$tgtDir/$id/results/result.aln" || !$new_only );
    } else {
      print "probabilistici elsi mej \n";
      system($call . " $tgtDir/$id/$id.fa")
      if ( !-e "$tgtDir/$id/results/result.aln" || !$new_only );
    #  system( $call . " $tgtDir/$id/$id.fa 1>$tgtDir/log/$id.locarna_call 2>$tgtDir/log/$id.locarna_call_err", $in_verbose ) == 0 or die " mloc command was unable to run to completion:\n\n";
      #if ( !-e "$tgtDir/$id/results/result.aln" || !$new_only );
    }
  #
    ...
    print "eval_tree_str = $eval_tree_str \n";
    chomp($eval_tree_str);
    print "eval_tree_str = $eval_tree_str \n";
  ...
  #     ## or if new structure is used: "intermediate_rootIntermID_idx.aln"
      ...
      ##ali fold
       ...
       chdir($tmp_dir);
       print "iidx = $iidx \n";
       #system("RNAalifold -r --noLP --color --aln $tgtDir/$id/intermediates/intermediate$iidx.aln 2>/dev/null ")== 0 or die " .RNAalifold. command was unable to run to completion:\n\n";
       #my @call_alifold = readpipe("RNAalifold -r --noLP --color --aln $tgtDir/$id/intermediates/intermediate$iidx.aln 2>/dev/null");
       my @call_alifold = readpipe("RNAalifold -r --noLP --color --aln ../../$tgtDir/$id/intermediates/intermediate$iidx.aln 2>/dev/null");
...
sub NodeScore {
  ...
  my @getmpi = readpipe("perl $path/scoreAln.pl -i $node->{FILE} -f CLUSTALW -s mpi");
  if (@getmpi) {
    $mpi = $getmpi[0];
    chomp($mpi) if ($mpi);
  }
...
sub check_node_overlap {
  ...
  my $matrix_ols = fragment_overlap( $node_frags, $node_frags, $black_overlap, 1 );
