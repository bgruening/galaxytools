        ...
        for filename in files:
            if os.path.splitext(filename)[-1] in [".gff3"]:
                ...
                gff2seq(
                    ...
                    False,
                    ncbi_traslation_table=traslation_table,
                )
...
class Segment:
    ...
    def __repr__(self):
        return (
            "Segment (%s:%s): [start: %s, end: %s, strand: %s, frame: %s, note: %s]"
            % (
                self.seq_type,
                self.iid,
                self.start,
...
class AugustusEntry:
    ...
    def add_segment(self, start, stop, strand, seq_type, frame, note=""):
        if seq_type == "CDS":
            self.seq_type = seq_type
            self.add_exon(int(start), int(stop), strand, self.name, int(frame), note)
        elif seq_type == "gene":
            self.seq_type = seq_type
    ...
    def add_mRNA(self, mRNA_start, mRNA_stop, strand, iid, note=""):
        ...
        self.mRNA = Segment(
            mRNA_start, mRNA_stop, strand, self.seq_type, "0", note, iid
        )
...
def run_blast(
    data_dir, blastdb="/media/mydata/uniprot/blastdb/uniprot_sprot.blastdb", threads=8
):
...
def run(data_dir, feature_table_path, locus_tag, min_coverage, min_ident):
    ...
    for iids in sorted_iids:
        ...
        if open(blastxml_file).read().strip() == "":
            continue
        augustus_mapping = get_augustus_mapping(
            blastxml_file.replace(".blastxml", ".gff3")
        )
...
def check_short_introns(cds):
    ...
    Please remove the CDS and mRNA features and annotate this with a single gene feature across the entire span.  Include the pseudo qualifier and a note on the gene with a brief description.   For example:
    1       200     gene
            gene    phoA
                ...
                for hsp in alignment.hsps:
                    ...
                    if ident > min_ident and coverage:
                        ...
                        if short_intron:
                            ...
                            feature_table_text[hsp.bits] += (
                                "\t\t\tnote\tnonfunctional; similar to %s\n" % accession
                            )
                            continue
                        """
