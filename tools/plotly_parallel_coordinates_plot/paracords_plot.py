...
def main(infile, col_dimensions, categorized, col_color, dimension_mode="by_index"):
    ...
    if dimension_mode not in ["by_index", "by_name"]:
        raise ValueError(
            "Select dimensions `{}` is not supported!".format(dimension_mode)
        )
    if dimension_mode == "by_index":
        ...
        if "*" not in col_dimensions:
            ...
            col_dimensions = [
                header for header in df.columns if re.search(pattern, header)
            ]

    for col_name in col_dimensions:
        values = df[col_name]
        if categorized == "boolfalse" and all(type(e) is int for e in values):
            dimensions.append(dict(values=values, tickformat=",.2r", label=col_name))
        elif categorized == "boolfalse" and all(type(e) is float for e in values):
            dimensions.append(dict(values=values, tickformat="g", label=col_name))
        else:
    ...
    dimensions.append(
        dict(values=colors, tickformat=tickformat, label=df.columns[col_color])
    )

    line = dict(color=colors, colorscale="Jet", showscale=True, reversescale=True)
