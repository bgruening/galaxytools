...
def binarize_mutations(df, gene_idx=1, sample_idx=2):
    ...
    if gene_idx >= len(df.columns) or sample_idx >= len(df.columns):
        raise ValueError(
            f"Column indices out of bounds. DataFrame has {len(df.columns)} columns, "
    ...
    if df[sample_col].isna().all():
        raise ValueError(
            f"Sample column (index {sample_idx}) contains only NaN values."
        )

    # Group by gene and sample, count mutations
    mutation_counts = (
        df.groupby([gene_col, sample_col]).size().reset_index(name="count")
    )

    # Create pivot table
    mutation_matrix = mutation_counts.pivot(
        index=gene_col, columns=sample_col, values="count"
    ).fillna(0)
...
def make_data_dict(clin_path, omics_paths):
    ...
    try:
        ...
        data["clin"] = clin
        print(
            f"Loaded clinical data: {clin.shape[0]} samples, {clin.shape[1]} features"
        )
    except Exception as e:
...
def validate_data_consistency(data):
    ...
    for name, df in data.items():
        ...
        if missing_in_omics:
            print(
                f"Warning: {len(missing_in_omics)} clinical samples not found in {name}"
            )
        if missing_in_clin:
            print(
                f"Warning: {len(missing_in_clin)} samples in {name} not found in clinical data"
            )
...
def split_and_save_data(data, ratio=0.7, output_dir="."):
    ...
    for key in data.keys():
        try:
            train_data[key].to_csv(
                os.path.join(output_dir, "train", f"{key}.tabular"), sep="\t"
            )
            test_data[key].to_csv(
                os.path.join(output_dir, "test", f"{key}.tabular"), sep="\t"
            )
        except Exception as e:
    ...
    for column_name in column_names:
        ...
        if require_integer:
            ...
            if not (non_null_values == non_null_values.round()).all():
                raise ValueError(
                    f"Column '{column_name}' contains non-integer numeric values."
                )
            print(
                f"Column '{column_name}': All values are integers or integer-equivalent floats."
            )
        else:
            print(
                f"Column '{column_name}': All values are numeric (integers and floats accepted)."
            )
...
def main():
    ...
    parser.add_argument(
        "--split", type=float, default=0.7, help="Train split ratio (default: 0.7)"
    )
    ...
    try:
        ...
        if args.util == "split":
            ...
            if not 0 < args.split < 1:
                raise ValueError(
                    f"Split ratio must be between 0 and 1, got {args.split}"
                )
            ...
            if not os.path.isfile(args.mutations):
                raise FileNotFoundError(
                    f"Mutation data file not found: {args.mutations}"
                )
            # Validate gene and sample indices
            if args.gene_idx < 0 or args.sample_idx < 0:
                raise ValueError(
                    "Gene and sample indices must be non-negative integers"
                )
        ...
        if args.util == "split":
            ...
            binarized_matrix = binarize_mutations(
                mutations_df, gene_idx=args.gene_idx, sample_idx=args.sample_idx
            )
