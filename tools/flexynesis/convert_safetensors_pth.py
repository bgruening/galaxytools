...
def build_minimal_dataset(config, artifacts):
    """
    Build a pickle-safe dataset-like object with the attributes expected
    by Flexynesis model constructors.
    """
    layers = config.get("layers", [])
    target_vars = config.get("target_variables", [])

    # Create features dict from artifacts: {layer_name: [feature1, feature2, ...]}
    features = artifacts["feature_lists"]
    ...
    for var in target_vars:
        if var not in variable_types:
            ...
            # Use one dummy value to indicate a single numerical output.
            ann[var] = np.array([0.0])

    # SimpleNamespace is pickle-safe and importable from stdlib during torch.load.
...
def reconstruct_model(safetensors_path, config_path, artifacts_path):
    ...
    dataset = build_minimal_dataset(config, artifacts)
