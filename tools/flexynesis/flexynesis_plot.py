...
def match_samples_to_embeddings(sample_names, labels):
    ...
    df_matched = sample_df.merge(
        labels, left_on="sample_names", right_on=first_column, how="left"
    )
...
def generate_dimred_plots(
    embeddings, matched_labels, args, output_dir, output_name_base
):
    ...
    if not args.color:
        if is_flexynesis_format:
            ...
            print(
                f"Generating {args.method.upper()} plots for known and predicted labels..."
            )
        else:
            print(
                "Labels are not in flexynesis format (Custom labels), please specify a color variable with --color"
            )
        ...
        print(
            f"Generating {args.method.upper()} plots for {len(target_values)} target variable(s): {', '.join(target_values)}"
        )
        ...
        if missing_vars:
            print(
                f"Warning: The following target variables were not found in the data: {', '.join(missing_vars)}"
            )
        ...
        if not valid_vars:
            raise ValueError(
                f"None of the specified target variables were found in the data. Available: {', '.join(available_vars)}"
            )
        ...
        for var in valid_vars:
            ...
            print(
                f"  Auto-detected color types - Known: {known_color_type}, Predicted: {predicted_color_type}"
            )
            ...
            try:
                ...
                output_path_known = (
                    output_dir / f"{output_name_base}_{var}_known.{args.format}"
                )
                ...
                output_path_predicted = (
                    output_dir / f"{output_name_base}_{var}_predicted.{args.format}"
                )
                print(
                    f"  Saving predicted labels plot to: {output_path_predicted.name}"
                )
                fig_predicted.save(
                    output_path_predicted, dpi=args.dpi, bbox_inches="tight"
                )
        ...
        if args.color not in matched_labels.columns:
            raise ValueError(
                f"Color variable '{args.color}' not found in matched labels. Available columns: {matched_labels.columns.tolist()}"
            )
...
def generate_km_plots(survival_data, labels, args, output_dir, output_name_base):
    ...
    if not is_flexynesis_format:
        raise ValueError(
            f"Labels are not in flexynesis format (Custom labels). Please provide a valid label file with the required columns, {required_cols}."
        )
    ...
    if survival_data.columns[0] != "sample_id":
        survival_data = survival_data.rename(
            columns={survival_data.columns[0]: "sample_id"}
        )
    ...
    if df_deceased.empty:
        raise ValueError(
            "No matching samples found after merging survival and label data."
        )
    ...
    output_path_known = (
        output_dir / f"{output_name_base}_km_risk_subtypes.{args.format}"
    )
...
def generate_cox_plots(
    ...
    is_flexynesis_format = all(
        col in important_features.columns for col in required_cols
    )

    if not is_flexynesis_format:
        raise ValueError(
            f"Labels are not in flexynesis format (Custom labels). Please provide a valid important_features file with the required columns, {required_cols}."
        )
    ...
    if all(
        var in clinical_train.columns and var in clinical_test.columns
        ...
        raise ValueError(
            f"Not all clinical variables found in datasets. Available in train dataset: {clinical_train.columns.tolist()}, Available in test dataset: {clinical_test.columns.tolist()}"
        )
    ...
    print(
        f"Extracting top {args.top_features} important features for {args.surv_event_var}..."
    )
    try:
        print(
            f"Loading {args.top_features} important features from: {args.important_features}"
        )
        imp_features = load_labels(args.important_features)
        imp_features = imp_features[
            imp_features["target_variable"] == args.surv_event_var
        ]
        if args.layer not in imp_features["layer"].unique():
            print(f"Available class labels: {imp_features['layer'].unique()}")
            raise ValueError(
                f"Class label '{args.layer}' not found in important features data: {args.important_features}"
            )
        imp_features = imp_features[imp_features["layer"] == args.layer]
        if imp_features.empty:
            raise ValueError(
                f"No important features found for target variable '{args.surv_event_var}' in {args.important_features}"
            )
    ...
    # Build Cox model
    print(
        f"Building Cox model with time variable: {args.surv_time_var}, event variable: {args.surv_event_var}"
    )
    try:
        coxm, metrics = build_cox_model(
            df,
...
def generate_plot_scatter(labels, args, output_dir, output_name_base):
    ...
    if is_flexynesis_format:
        ...
        for target_value in target_values:
            ...
            if target_labels.empty:
                print(
                    f"  Warning: No data found for target value '{target_value}' - skipping"
                )
                skipped_plots += 1
                continue
            ...
            predicted_values = pd.to_numeric(
                target_labels["predicted_label"], errors="coerce"
            )

            if true_values.isna().all() or predicted_values.isna().all():
                print(
                    f"No valid numeric values found for known or predicted labels in '{target_value}'"
                )
                skipped_plots += 1
                continue
            ...
            try:
                ...
                safe_target_name = (
                    target_value.replace("/", "_").replace("\\", "_").replace(" ", "_")
                )
        ...
        if successful_plots == 0:
            raise ValueError(
                "No scatter plots could be generated. Check your data and target values."
            )
    ...
    if not is_flexynesis_format:
        ...
        if not args.true_label or not args.predicted_label:
            raise ValueError(
                "For custom labels, please specify --true_label and --predicted_label arguments."
            )
...
def generate_pr_curves(labels, args, output_dir, output_name_base):
    ...
    for target_value in target_values:
        ...
        if non_na_probs < len(target_labels) * 0.1:  # Less than 10% valid probabilities
            print(
                "  Detected regression problem - precision-recall curves not applicable"
            )
            print(
                f"  Skipping '{target_value}' (use regression evaluation metrics instead)"
            )
            continue
        ...
        print(
            f"  Data summary: {total_rows} total rows, {unique_samples} unique samples, {unique_classes} unique classes"
        )
        print(
            f"  Missing data: {missing_labels} missing known_label, {missing_probs} missing probability"
        )

        if missing_labels > 0:
            print(f"  Warning: Found {missing_labels} missing known_label values")
            missing_samples = target_labels[target_labels["known_label"].isna()][
                "sample_id"
            ].unique()[:5]
            ...
            if target_labels.empty:
                print(
                    f"  Error: No valid known_label data remaining for '{target_value}' - skipping"
                )
                continue

        # 1. Pivot to wide format
        prob_df = target_labels.pivot(
            index="sample_id", columns="class_label", values="probability"
        )
        ...
        if nan_counts.any():
            ...
            print(
                f"  Samples with any NaN: {prob_df.isna().any(axis=1).sum()}/{len(prob_df)}"
            )
            ...
            if all_nan_rows.any():
                print(
                    f"  Dropping {all_nan_rows.sum()} samples with all NaN probabilities"
                )
            ...
            if remaining_nans > 0:
                print(
                    f"  Warning: {remaining_nans} individual NaN values remain - filling with 0"
                )
                prob_df = prob_df.fillna(0)

            if prob_df.empty:
                print(
                    f"  Error: No valid probability data remaining for '{target_value}' - skipping"
                )
                continue

        # 2. Get true labels
        true_labels_df = target_labels.drop_duplicates("sample_id")[
            ["sample_id", "known_label"]
        ].set_index("sample_id")
        ...
        if len(common_indices) == 0:
            print(
                f"  Error: No common sample_ids between probability and true label data for '{target_value}' - skipping"
            )
            continue

        print(
            f"  Found {len(common_indices)} samples with both probability and true label data"
        )
        ...
        if y_true.isna().any():
            print(
                f"  Error: True labels still contain NaN after alignment for '{target_value}' - skipping"
            )
            continue

        if prob_df_aligned.isna().any().any():
            print(
                f"  Error: Probability data still contains NaN after alignment for '{target_value}' - skipping"
            )
            continue
        ...
        print(
            f"  Label distribution: {dict(zip(*np.unique(y_true_np, return_counts=True)))}"
        )
        ...
        try:
            ...
            print(
                f"  Error generating precision-recall curve for '{target_value}': {str(e)}"
            )
            print(
                f"  Debug info - y_true type: {type(y_true_np)}, contains NaN: {pd.isna(y_true_np).any()}"
            )
            print(
                f"  Debug info - y_probs type: {type(y_probs_np)}, contains NaN: {pd.isna(y_probs_np).any()}"
            )
            continue
    ...
    for target_value in target_values:
        ...
        for class_label in classes:
            ...
            try:
                ...
                print(
                    f"      Error generating box plot for class '{class_label}': {str(e)}"
                )
                continue
...
def main():
    ...
    parser.add_argument(
        "--color", type=str, default=None, help="User-defined color for the plot."
    )
    ...
    parser.add_argument(
        "--target_value", type=str, default=None, help="Target value for scatter plot."
    )
    ...
    parser.add_argument(
        "--output_name", type=str, default=None, help="Output filename base"
    )
    parser.add_argument(
        "--format",
        type=str,
    ...
    parser.add_argument(
        "--dpi", type=int, default=300, help="DPI for the output image. Default is 300"
    )
    ...
    try:
        ...
        if args.plot_type not in [
            ...
            raise ValueError(
                f"Invalid plot type: {args.plot_type}. Must be one of: 'dimred', 'kaplan_meier', 'cox', 'scatter', 'concordance_heatmap', 'pr_curve', 'roc_curve', 'box_plot'"
            )
        ...
        if args.plot_type in ["dimred"]:
            ...
            if not args.labels:
                raise ValueError(
                    "--labels is required for dimensionality reduction plots"
                )
            if not args.method:
                raise ValueError(
                    "--method is required for dimensionality reduction plots"
                )

        if args.plot_type in ["kaplan_meier"]:
            if not args.survival_data:
                raise ValueError(
                    "--survival_data is required when plot_type is 'kaplan_meier'"
                )
            if not os.path.isfile(args.survival_data):
                raise FileNotFoundError(
                    f"Survival data file not found: {args.survival_data}"
                )
            if not args.labels:
                raise ValueError(
                    "--labels is required for dimensionality reduction plots"
        ...
        if args.plot_type in ["cox"]:
            if not args.important_features:
                raise ValueError(
                    "--important_features is required when plot_type is 'cox'"
                )
            if not os.path.isfile(args.important_features):
                raise FileNotFoundError(
                    f"Important features file not found: {args.important_features}"
                )
            ...
            if not os.path.isfile(args.clinical_train):
                raise FileNotFoundError(
                    f"Training dataset file not found: {args.clinical_train}"
                )
            ...
            if not os.path.isfile(args.clinical_test):
                raise FileNotFoundError(
                    f"Test dataset file not found: {args.clinical_test}"
                )
        ...
        if args.plot_type in ["scatter"]:
            ...
            if not args.target_value:
                print(
                    "--target_value is not specified, using all unique variables from labels"
                )
        ...
        if args.plot_type in ["dimred"]:
            ...
            generate_label_concordance_heatmap(
                labels, args, output_dir, output_name_base
            )
