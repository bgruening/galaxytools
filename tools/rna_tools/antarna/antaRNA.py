    ...
    for id1 in basepairstack:  # key = (constraint , (pos, constraint)))
        ...
        if id1 != id2 and not isCompatible(constr1, constr2, IUPAC_compatibles):
            ...
            returnstring += (
                ...
                + " at position "
                + str(id1)
                + " is not compatible with nucleotide constraint "
                + str(constr2)
                + " at position "
                + str(id2)
                + "\n"
...
def getLP(BPSTACK):
    ...
    for i in stack.keys():
        if (
            ...
            and (i + 1 in stack and stack[i + 1] == stack[i] - 1)
            and not (i + 2 in stack and stack[i + 2] == stack[i] - 2)
        ):
            LP[i] = stack[i]
            LP[i + 1] = stack[i + 1]
    ...
    for i in xrange(len(s)):
        if s[i] in "(){}[]<>":
            ...
            # closing
                ...
                bpstack[
                    no
                ] = i  # save basepair in the format {opening base id (opening seq constr,(closing base id, closing seq constr))}
                bpstack[
                    i
                ] = no  # save basepair in the format {closing base id (closing seq constr,(opening base id, opening seq constr))}

        elif (
            s[i] == "."
        ):  # no structural constaint given: produce entry, which references itself as a base pair partner....
            bpstack[i] = i
...
def getConstraint(TE, BPstack, IUPAC, IUPAC_compatibles, IUPAC_reverseComplements):
    ...
    val = BPstack[
        id1
    ]  # check out the value of the destination character in the basepair/constraint stack
    ...
    c2 = set(
        IUPAC_reverseComplements[constr2]
    )  # getting the reverse complement explicit symbols of c2
...
def applyTerrainModification(
    terrain, s, tmpGC, SC, BPstack, IUPAC, IUPAC_compatibles, IUPAC_reverseComplements
):
    ...
    for terrainelement in sorted(terrain):
        ...
        pheromone = getConstraint(
            terrainelement, BPstack, IUPAC, IUPAC_compatibles, IUPAC_reverseComplements
        )
        pathlength = getConstraint(
            terrainelement, BPstack, IUPAC, IUPAC_compatibles, IUPAC_reverseComplements
        )
        pathlength = applyGCcontributionPathAdjustment(
            pathlength, tmpGC, terrainelement.split(".")[1][-1:]
        )
        if pheromone * pathlength == 0:
            dels.append(terrainelement)
...
def pickStep(tmp_steps, summe):
    ...
    if len(tmp_steps) == 1:
        ...
        for choice in xrange(len(tmp_steps)):
            ...
            if (
                mainval > rand
            ):  # as soon, as the mainval gets larger than the random value the assignment is done
                return label
...
def getStructuralDistance(
    ...
    for lp in LP:
        if (
            bp[lp] == LP[lp]
        ):  # if the base pair is within the current solution structure, re-add the basepair into the constraint structure.
    ...
    for c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
        ...
        for m in re.finditer(
            c, target_structure
        ):  # search for a declarator in the requested structure
    ...
    IUPACinducers, tmp_Cseq = getInducingSequencePositions(
        Cseq, degreeOfSequenceInducement
    )
    if len(Cseq.strip("N")) > 0:
        ...
        for i in BP:  # Check for each base index i
            ...
            if (
                i < bp[i]
            ):  # if the current index is samller that the affiliated in the basepair stack of the current solution

                bp_j = bp[i]  # Actual j index of the current solution
                BP_j = BP[i][1][0]  # j index of the requested structure
                if (
                    i != bp_j and i == BP_j and BP[i][0] in IUPACinducers
                ):  # if i pairs with some other base in the current structure, and i is requested single stranded and the Sequence constraint is allowed to induce...
                    if (
                        BP[bp_j][1][0] == bp_j and BP[bp_j][0] in IUPACinducers
                    ):  # If position j is requested singlestranded and position j nucleotide can induce base pairs
    ...
    for lp in LP:
        ...
        if bp[lp] != LP[lp]:
            ...
            if not (
                isStru and isStru
            ):  # check if the bases at the specific positions are compatible and check if the
...
def trailBlaze(p, c_s, s, ds, dgc, dseq, dn, t, correction_terms, BPstack, verbose):
    ...
    for trans in xrange(len(transitions)):  # for each transition in the path
        ...
        multiplicator = 0
        if (
            tar_id2 == curr_id2 and id1 != tar_id2 and id1 != curr_id2
        ):  # case of a base pair, having both brackets on the correct position
            multiplicator = 1
        elif (
            tar_id2 == curr_id2 and id1 == tar_id2 and id1 == curr_id2
        ):  # case of a single stranded base in both structures
            multiplicator = 1
        p, l, c = terr[
            transitions[trans]
        ]  # getting the pheromone and the length value of the single path transition
        p += d * multiplicator
...
def updateTerrain(
    p, c_s, s, ds, dgc, dseq, dn, t, er, correction_terms, BPstack, verbose, ant_count
):
    """
...
def runColony(
    ...
    START_constraint_compatibility, CompReport = checkConstaintCompatibility(
        BPstack, SC, IUPAC_compatibles
    )
    ...
    if g2g == 1:
        ...
        # print len(terrain),
        terrain = applyTerrainModification(
            terrain,
            s,
            GC,
            SC,
        ...
        while criterion != met and getUsedTime(start_time) < max_time:
            ...
            path_info = getPathFromSelection(
                ants_per_selection,
                s,
                terrain,
                alpha,
            ...
            if improve == "h":  # hierarchical check
                ...
                if distance_structural < best_solution[3] or (
                    distance_structural == best_solution[3]
                    and distance_GC < best_solution[4]
                ):
                    best_solution = curr_solution
                    ant_no = 1
                # for the local (reset) best solution
                if distance_structural < best_solution_local[3] or (
                    distance_structural == best_solution_local[3]
                    and distance_GC < best_solution_local[4]
                ):
            ...
            if inConvergenceCorridor(
                curr_solution[3],
                curr_solution[4],
                best_solution_local[3],
                best_solution_local[4],
            ):
                convergence_counter += 1
            if (
                distance_structural_prev == distance_structural
                and distance_GC_prev == distance_GC
            ):
                convergence_counter += 1
            ...
            if (
                ant_no == termination_convergence
                or resets >= reset_limit
                or global_ant_count >= 100000
                or best_solution_since == 5
            ):
                break
        ...
        retString += (
            ...
            + str(START_constraint_compatibility)
            + "\n"
            + CompReport
            + "\n"
        )
    ...
    for col in xrange(colonies):
        ...
        if output_verbose:
            line += (
                ...
                + str(GC)
                + "|ER:"
                + str(evaporation_rate)
                + "|Struct_CT:"
                + str(struct_correction_term)
                + "|GC_CT:"
                + str(GC_correction_term)
                + "|Seq_CT:"
                + str(seq_correction_term)
                + output_v
                + "\n"
                + "\n".join(output_w)
            )
...
def checkForViennaTools():
    ...
    """
    RNAfold_output = (
        subprocess.Popen(["which", "RNAfold"], stdout=subprocess.PIPE)
        .communicate()[0]
        .strip()
    )
    if (
        len(RNAfold_output) > 0
        and RNAfold_output.find("found") == -1
        and RNAfold_output.find(" no ") == -1
    ):
        return True
    else:
        print(
...
def checkForpKiss():
    ...
    """
    pKiss_output = (
        subprocess.Popen(["which", "pKiss"], stdout=subprocess.PIPE)
        .communicate()[0]
        .strip()
    )
    if (
        len(pKiss_output) > 0
        and pKiss_output.find("found") == -1
