...
class RBPVectorizer:
    ...
    def __init__(self, fasta):
        ...
        self._reference_fisher_scores = "AURA_Human_data/RBP_features/fisher_scores_ref"
        self._train_rbps_file = "AURA_Human_data/RBP_features/rbps_in_train.txt"
    ...
    def _overlapping_domains(self):
        ...
        reference_domains = set(
            [
                dom.replace(".mod", "")
                for dom in listdir(self._mod_fold)
                if isfile(join(self._mod_fold, dom))
            ]
        )
    ...
    def _compute_fisher_scores(self, dom_list):
        ...
        def get_fisher_scores(dom_list, mod_fold, dom_fold, fisher_fold):
            for acc in dom_list:
                ...
                fisher = sp.check_output(cmd, shell=True, stderr=_FNULL)
                nf = open("%s/%s.txt" % (fisher_fold, acc), "w")
                nf.write(fisher)
                nf.close()

        mkdir(self._fisher_fold)
        get_fisher_scores(dom_list, self._mod_fold, self._dom_fold, self._fisher_fold)
    ...
    def _ekm(self, dom_list):
        ...
        f.close()
        ref = dom_features(self._reference_fisher_scores, dom_list, names=train_rbps)
