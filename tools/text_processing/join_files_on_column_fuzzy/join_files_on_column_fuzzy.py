            ...
            while cache:
                ...
                if args.unit == "absolute":
                    if value2 <= upper_bound and value2 >= (value1 - args.distance):
                        ...
                        write_buffer.append(
                            [abs_dist, line_template % "\t".join(columns + _c)]
                        )
                    ...
                    if ppm_dist <= args.distance:
                        ...
                        write_buffer.append(
                            [ppm_dist, line_template % "\t".join(columns + _c)]
                        )
                        _cache.append([_c, value2])
                        fill_cache()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Merge two files on a common column the fuzzy way."
    )
    parser.add_argument("--f1", required=True)
    parser.add_argument("--f2", required=True)
    parser.add_argument(
        "--c1", type=int, required=True, help="Column in file 1 to be merged on."
    )
    parser.add_argument(
        "--c2", type=int, required=True, help="Column in file 2 to be merged on."
    )
    parser.add_argument("--outfile", required=True)
    parser.add_argument(
        "--header",
        action="store_true",
        help="The files have a header line at the beginning.",
    )
    parser.add_argument(
        "--closest", action="store_true", help="Only report the closest match."
    )
    parser.add_argument(
        "--add_distance",
        action="store_true",
        help="Add addional column with the distance between the two values.",
    )
    parser.add_argument(
        "--sep", type=str, default="\t", help="Files are separated by this separator."
    )
    parser.add_argument(
        "--distance", type=float, default="0.2", help="Maximal allowed distance."
    )
