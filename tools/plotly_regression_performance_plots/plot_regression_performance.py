...
def main(infile_input, infile_output):
    ...
    trace_true = go.Scatter(
        x=axis_labels, y=true_values, mode="lines+markers", name="True values"
    )

    trace_predicted = go.Scatter(
        x=axis_labels, y=predicted_values, mode="lines+markers", name="Predicted values"
    )
    ...
    rmse = np.sqrt(
        np.mean([(x - y) ** 2 for x, y in zip(true_values, predicted_values)])
    )

    trace_x_eq_y = go.Scatter(
        x=x_y_values, y=x_y_values, mode="lines", name="X = Y curve"
    )
    ...
    layout_true_pred = go.Layout(
        title="True vs predicted values (RMSE: %s, R2: %s)"
        % (str(np.round(rmse, 2)), str(np.round(r2, 2))),
    ...
    plotly.offline.plot(
        fig_true_pred, filename="output_scatter_plot.html", auto_open=False
    )

    # residual plot
    residual = predicted_values - true_values
    trace_residual = go.Scatter(x=predicted_values, y=residual, mode="markers")
    ...
    plotly.offline.plot(
        fig_residual, filename="output_residual_plot.html", auto_open=False
    )
