...
class ModelDictionary(object):
    ...
    def from_dict(cls, data):
        ...
        if t is list:
            newlist = []
            for e in data:
                newlist.append( cls.from_dict(e) )
            return newlist
        if t is tuple:
            return tuple( cls.from_dict(list(data)) )
        if t is set:
            return set( cls.from_dict(list(data)) )
        if t is dict:
