...
def printData(data, gffFile, source, write):
    ...
    with open(gffFile,write) as file:
        for entry in data:
            print entry
            print bp.to_seqfeature(entry)
            fasta_input = SeqIO.to_dict(SeqIO.parse('./test-data/FlyBase.fna', "fasta", generic_dna))
            gff_iter = GFF.parse(gff_file, fasta_input)

            SeqIO.write( [bp.to_seqfeature(entry)], 'ultimate_test.gb', 'genbank')
