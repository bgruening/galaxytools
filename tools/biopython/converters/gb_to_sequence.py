...
def extract_features(
    gb_iter, feature_name, transl_table, is_complete_cds, stop, to_protein
):
    ...
    for gb_record in gb_iter:
        ...
        for feature in gb_record.features:
            if feature.type == feature_name:
                ...
                if not to_protein:
                    seq = SeqRecord(
                        gene, id=feature_id, name=feature_id, description=desc
                    )
                else:
                    seq = SeqRecord(
                        gene.translate(
                            table=transl_table, to_stop=True, cds=is_complete_cds
                        ),
                        id=feature_id,
                        name=feature_id,
                        description=desc,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="gbk_to_orf.py", description="Extract features from GenBank files."
    )
