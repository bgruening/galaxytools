...
import numpy as np
import pandas as pd
from sklearn.metrics import average_precision_score, precision_recall_curve, root_mean_squared_error, r2_score
...
def train_evaluate(args):
    ...
    if args["selected_task"] == "Classification":
        ...
        if len(te_labels) > 0:
            ...
            classification_plot(recall, precision, f'Precision-Recall Curve (AP={average_precision:.2f})', 'Precision-Recall Curve', 'Recall', 'Precision')
    else:
        ...
        if len(te_labels) > 0:
            ...
            regression_plot(te_labels, y_eval, f'Scatter plot: True vs predicted values. RMSE={score:.2f}, R2={r2_metric_score:.2f}', 'True values', 'Predicted values')
    e_time = time.time()
    print("Time taken by TabPFN for training and prediction: {} seconds".format(e_time - s_time))
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("-testhaslabels", "--testhaslabels", required=True, help="if test data contain labels")
    arg_parser.add_argument("-selectedtask", "--selected_task", required=True, help="Type of machine learning task")
    # get argument values
