...
class DecisionBoundaryDisplay:
    ...
    def plot(self, plot_method="contourf", ax=None, xlabel=None, ylabel=None, **kwargs):
        ...
        **kwargs : dict
        ...
        plt.savefig("output_decision_boundary.png")
        return self
...
def make_decision_boundaries(trained_model, tr_features, tr_labels):
    # PLOTTING
    fig = plt.figure(figsize=(10,10))
    ax = fig.add_subplot(111)
    cm = plt.cm.RdBu
    cm_bright = ListedColormap(["purple", "yellow"])
    trained_model.fit(tr_features[:, 0:2], tr_labels)

    DecisionBoundaryDisplay.from_estimator(
        trained_model, tr_features[:, 0:2], alpha=0.6, ax=ax, eps=2.0, grid_resolution=25, response_method="predict_proba"
    )
    ax.scatter(tr_features[:, 0], tr_features[:, 1], c=tr_labels > 0, cmap=cm_bright)
...
def train_evaluate(args):
    ...
    make_decision_boundaries(classifier, tr_features, tr_labels)
