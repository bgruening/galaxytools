...
process_pair <- function(sc_data, bulk_data,
    ...
    ## -
    return(list(
        est_music = estimated_music_props,
...
group_by_dataset <- function(summat) {
    ...
    return(list(
        prop = as.data.frame(bd),
        scaled = as.data.frame(bd_scale),
        spread = list(
            scale = bd_spread_scale,
            prop = bd_spread_prop
        )
    ))
}
...
summarize_heatmaps <- function(grudat_spread_melt, do_factors, cluster = "None") {
    ...
    do_single <- function(grudat_melted, yaxis, xaxis, fillval, title,
        ...
        if (cluster == "None") {
            ...
            return(do_cluster(
                grudat_spread_melt, xaxis, yaxis, fillval,
                xlabs, ylabs, title,
...
summarize_boxplots <- function(grudat_spread, do_factors) {
    common1 <- ggplot(grudat_spread, aes(x = value.prop)) +
        ggtitle("Sample") +
        xlab(element_blank()) +
        ylab(element_blank())
    common2 <- ggplot(grudat_spread, aes(x = value.scale)) +
        ggtitle("Reads") +
        xlab(element_blank()) +
        ylab(element_blank())
...
grudat_spread_melt <- merge_factors_spread(
    grudat$spread,
...
heat_maps <- summarize_heatmaps(
    grudat_spread_melt_filt, do_factors,
    dendro_setting
)
...
colnames(grudat_spread_melt) <- c(
    "Sample", "Cell", "Bulk", "Factors",
