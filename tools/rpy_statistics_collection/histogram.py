...
def main():
    ...
    if skipped_lines < i:
        try:
            ...
            r.pdf(out_fname, 8, 8)
            histogram = r.hist(
                a, probability=not frequency, main=title, xlab=xlab, breaks=breaks
            )
            if density:
                density = r.density(a)
                if frequency:
                    scale_factor = len(matrix) * (
                        histogram["mids"][1] - histogram["mids"][0]
                    )  # uniform bandwidth taken from first 2 midpoints
