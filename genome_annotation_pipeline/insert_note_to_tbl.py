...
def main(tbl_file, discrep_file, outfile):
    ...
    for line in open(discrep_file):
        ...
        if start:
            ...
            temp_dict = {'discrep_id': discrep_id, 'name': name, 'seq_pos': seq_pos, 'iids': [iid]}
            if discrep_overlapping_list and discrep_overlapping_list[-1]['name'] == name:
                ...
                for iid in group['iids']:
                    if line == 'protein_id\tgnl|PBUF|%s' % iid:
                        ...
                        if len(group_exclude) == 1:
                            note_line = '			note	overlaps CDS with %s\n' % group_exclude[0]
                        elif len(group_exclude) == 2:
                            note_line = '			note	overlaps CDS with %s and %s\n' % (group_exclude[0], group_exclude[1])
                        else:
                            note_line = '			note	overlaps CDS with %s and %s\n' % (', '.join( group_exclude[:-1] ), group_exclude[-1])
                        found_product = False
...
if __name__ == '__main__':
    ...
    parser.add_argument("--tbl", required=True,
                    help="NCBI table file.")

    parser.add_argument("--discrep", required=True,
                    help="NCBI discrepancy file.")

    parser.add_argument('-o', '--outfile', required = True)

    parser.add_argument('-t', '--test', action='store_true', default=False, help='Run tests.')
